...
class Solution:
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        Bar = namedtuple("Bar", ["height", "index"])
        n = len(heights)
        
        hmap = {v : k for k, v in enumerate(heights)}
        ...
        for i, h in enumerate(heights):
            ...
            while len(mono) > 0 and mono[-1].height > h:
                h_prev = mono.pop()
                # print(f"found {h=}, the next shortest for {h_prev=}")
                shortest_left[h_prev.index] = i # next shortest found
            # we want the index at which h exists
            mono.append(Bar(height=h, index=i))
        # print(f"evaluating {h=}, with {mono=}, {shortest_left=}")
