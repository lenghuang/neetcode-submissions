...
class Solution:
    ...
    def recurse(self, nums: List[int], acc: List[int], t: int, global_res: List[List[int]]):
        # print("recurse -- nums=", nums, "t=", t, "acc=", acc)
        if t == 0 and len(acc) > 0:
            # print("NUMS IS 0 -- appending :)")
            global_res.append(acc)
            return
        elif len(nums) == 0 or t < 0:
        ...
        for k in range((t // head) + 1):
            ...
            self.recurse(tail, new_acc, t - new_num, global_res)
    
    def combinationSum(self, nums: List[int], target: int) -> List[List[int]]:
        global_res = []
        self.recurse(nums, [], target, global_res)
        return global_res
